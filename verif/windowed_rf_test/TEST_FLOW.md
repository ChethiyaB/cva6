...
This compiles the RTL (including windowed regfile and CSRs 0x800/0x801/0x802, apply-on-mret) into the executable `work-ver/Variane_testharness`.  
...
Build the minimal CSR test (see below), then run with the test ELF. **Required:** pass `+boot_addr=0x80000000` so the core boots at the loaded ELF (default is ROM at 0x10000, which is not loaded and causes B Response Errored). Also pass the tohost address for the tracer:

```bash
make sim-verilator elf_file=$CVA6_REPO_DIR/verif/windowed_rf_test/build/windowed_rf_csr_test.riscv \
  SIM_ARGS="+boot_addr=0x80000000"
```
...
- You should see `[ariane_testharness] boot_addr overridden to 0x80000000` if the boot plusarg was applied.
- **tohost** is at **0x80001000**; the tracer and DTM now find it directly from the ELF symbols (no `+tohost_addr` needed).

Without `+boot_addr=0x80000000` the core starts at ROM (0x10000); that region is not loaded by the ELF, so the core runs garbage and the first store can trigger **B Response Errored**.
...
- Reads CSR 0x800 (active window base) and 0x801 (active window size); checks reset values (0 and 32).
- Writes staged values to 0x800/0x801, applies with 0x802, then reads back (Phase 2 semantics).
...
Then run with `elf_file` and `SIM_ARGS="+boot_addr=0x80000000"` as in 4.4 above. That tells the tracer where the test writes the exit status so the run can finish with pass/fail instead of timing out. Passing the CSR read/write checks indicates the window CSRs are working.
