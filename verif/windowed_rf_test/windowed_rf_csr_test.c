/**
 * Full CSR window test for windowed RF (Phase 1 + Phase 2).
 * Phase 2: 0x800/0x801 write staged config; 0x802 apply staged -> active.
 * Validates: reset (0,32), staged base apply/read/restore, staged size apply/read/restore, final (0,32).
 * Naked main with single asm block; tail-calls write_tohost(1)=pass, write_tohost(3)=fail.
 */
